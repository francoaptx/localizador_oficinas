  ...
  return (
    <BottomSheet
      ...
      snapPoints={({ minHeight, maxHeight }) => [
        minHeight, // Altura mínima (solo el encabezado)
        maxHeight * 0.4, // 40% de la altura de la pantalla
        maxHeight * 0.9, // 90% de la altura de la pantalla
      ]}
      ...
      header={
        <div className="p-3 w-full">
          ...
          <h3 className="text-sm font-medium text-center text-gray-600">
            {isLoading ? 'Buscando...' : `${offices.length} oficinas encontradas`}
          </h3>
        </div>
      }
      className="z-[1000]" // Asegura que esté sobre el mapa
      style={{ '--rsbs-content-height': '100%' } as React.CSSProperties} // Permite que el contenido ocupe toda la altura
    >
